...
class G1BoxGraspEnv(gym.Env):
    ...
    def __init__(self, render_mode='human', render_fps=30, policy_freq=50):
        ...
        self.max_safe_grip_force = self.min_grip_force * 2.0  # Don't exceed 2x
        ...
        self.locked_actuators = list(range(0, 15))  # Legs (0-11) + Waist (12-14)
        self.left_arm_actuators = [15, 16, 17, 18]  # shoulder pitch/roll/yaw, elbow
        self.right_arm_actuators = [29, 30, 31, 32]  # shoulder pitch/roll/yaw, elbow
        self.left_wrist_actuators = [19, 20, 21]  # wrist roll/pitch/yaw
        self.right_wrist_actuators = [33, 34, 35]  # wrist roll/pitch/yaw
        self.hand_actuators = list(range(22, 29)) + list(range(36, 43))  # All fingers
        
        # Mirror signs for symmetric control (test and adjust if needed)
        self.arm_mirror_signs = np.array([1, -1, -1, 1])  # pitch, roll, yaw, elbow
    ...
    def get_obs(self):
        ...
        """
        # Left arm state
        left_arm_qpos_indices = [22, 23, 24, 25]  # In full qpos array
        left_arm_qvel_indices = [21, 22, 23, 24]  # In full qvel array
        ...
        # Hand positions
        left_hand_pos = self.data.xpos[self.left_hand_body_id]
        right_hand_pos = self.data.xpos[self.right_hand_body_id]
    ...
    def calculate_reward(self):
        ...
        target_left = box_pos + np.array([0, -self.box_half_depth, 0])
        target_right = box_pos + np.array([0, self.box_half_depth, 0])

        left_dist = np.linalg.norm(left_hand_pos - target_left)
        right_dist = np.linalg.norm(right_hand_pos - target_right)
        
        # Use exponential decay for smoother reward
        reaching_reward = np.exp(-2.0 * left_dist) + np.exp(-2.0 * right_dist)
        reward += w_reach * reaching_reward

        # 2. CONTACT REWARD: Individual contacts + bilateral bonus
        ...
        # Individual contact rewards
        left_contact_reward = 5.0 if left_touching else 0.0
        right_contact_reward = 5.0 if right_touching else 0.0
        reward += w_contact * (left_contact_reward + right_contact_reward)
        
        # Bilateral contact bonus
    ...
    def step(self, action):
        ...
        self.data.ctrl[self.left_wrist_actuators] = self.standing_ctrl[self.left_wrist_actuators]
        self.data.ctrl[self.right_wrist_actuators] = self.standing_ctrl[self.right_wrist_actuators]
