...
class G1BoxGraspEnv(gym.Env):
    ...
    def calculate_reward(self, action):
        ...
        3. Grip / fingers: more fingers on the box is better
        4. Lift height: box height progress (coupled with uprightness)
        5. Stability: box position/orientation stable
        6. Wrist health: penalize collapsed wrists when lifting
        7. Control cost: penalize excessive actions
        8. Alive bonus: small reward for staying alive
        """
        reward = 0.0
        ...
        w_reach      = 15.0
        w_contact    = 5.0
        # w_force    = 2.0   # currently unused (no force term yet)
        w_lift       = 10.0
        w_stability  = 0.5
        w_control    = 0.1
        w_alive      = 0.1
        w_vel        = 0.0   # global velocity penalty
        w_fingers    = 3.0   # more fingers touching
        w_hold       = 5.0   # per-step reward for holding box at target
        w_tilt       = 6.0   # explicit tilt penalty (rad)
        w_wrist_ang  = 4.0   # penalize wrist bending when lifting
        ...
        # Box orientation quaternion (w, x, y, z)
        box_quat = self.data.qpos[self.box_qpos_start+3:self.box_qpos_start+7].copy()

        # Convert quaternion -> rotation matrix to compute tilt
        mat = np.zeros(9, dtype=np.float64)
        mujoco.mju_quat2Mat(mat, box_quat)
        R_box = mat.reshape(3, 3)
        box_up = R_box[:, 2]                       # box's +z in world frame
        world_up = np.array([0.0, 0.0, 1.0])
        cos_tilt = np.clip(box_up @ world_up, -1.0, 1.0)
        tilt_angle = np.arccos(cos_tilt)           # 0 = perfectly upright

        # ------------------------------------------------------------------
        # Hand "palm" positions
        # ------------------------------------------------------------------
        left_thumb_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_BODY, "left_hand_thumb_0_link"
        )
        left_index_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_BODY, "left_hand_index_0_link"
        )
        left_middle_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_BODY, "left_hand_middle_0_link"
        )

        right_thumb_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_BODY, "right_hand_thumb_0_link"
        )
        right_index_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_BODY, "right_hand_index_0_link"
        )
        right_middle_id = mujoco.mj_name2id(
        ...
        # ------------------------------------------------------------------
        # Global velocity penalty (optional)
        # ------------------------------------------------------------------
        ...
        def reach_term(hand_pos, target_pos):
            ...
            if hand_z > z_low:
                # Above box - strong gradient
                return 50.0 - 20.0 * dist
            else:
                # Below box - weak gradient so we don't encourage table contact
        ...
        reward += w_reach * (left_reach + right_reach)

        # ------------------------------------------------------------------
        # 2. CONTACT + FINGER CONTACT REWARD
        # ------------------------------------------------------------------
        left_touching = any(
            self.check_contact(body, "box_geom") for body in self.left_hand_bodies
        )
        right_touching = any(
            self.check_contact(body, "box_geom") for body in self.right_hand_bodies
        )

        # Basic contact reward (binary)
        contact_reward = 0.0
        if left_touching:
            contact_reward += 1.0
        if right_touching:
        ...
        if left_touching and right_touching:
            contact_reward += 1.0  # bilateral bonus

        reward += w_contact * contact_reward

        # Fingers: count distinct finger bodies touching the box
        ...
        fraction_fingers = (
            finger_contact_count / max_fingers if max_fingers > 0 else 0.0
        )
        reward += w_fingers * fraction_fingers

        # ------------------------------------------------------------------
        # 3–5. LIFT, HOLD, STABILITY (only when grasped)
        # ------------------------------------------------------------------
        ...
        # Upright factor in [0,1], 1 if perfectly upright
        upright_factor = max(np.cos(tilt_angle), 0.0)

        if left_touching and right_touching:
            # 4. LIFT HEIGHT (coupled with uprightness)
            ...
            totalHeightReward = w_lift * (progress_reward + target_bonus) * upright_factor
            reward += totalHeightReward

            # 4b. HOLD REWARD: per-step bonus when near target height
            if height_error < 0.05:
                reward += w_hold

            # 5. STABILITY: only once lifted a bit
            if height_progress > 0.05:
                # Low box velocity (linear)
                box_vel = self.data.qvel[self.box_qvel_start:self.box_qvel_start+6]
                box_speed = np.linalg.norm(box_vel[:3])
                velocity_reward = np.exp(-2.0 * box_speed)

                # Keep box near initial XY
                xy_drift = np.linalg.norm(box_pos[:2] - self.initial_box_pos[:2])
                position_reward = np.exp(-5.0 * xy_drift)

                # Old orientation reward term (kept, but now weaker)
                quat_upright_reward = 2.0 * abs(box_quat[0])

                stability_reward = (
                    velocity_reward + position_reward + quat_upright_reward
                )
                reward += w_stability * stability_reward

        # ------------------------------------------------------------------
        # 6. WRIST HEALTH: penalize collapsed wrists when lifted
        # ------------------------------------------------------------------
        if height_progress > 0.02:
            q = self.data.qpos

            # NOTE: these attributes MUST be defined in __init__
            wrist_qpos_indices = [
                self.left_wrist_pitch_qpos_idx,
                self.left_wrist_yaw_qpos_idx,
                self.right_wrist_pitch_qpos_idx,
                self.right_wrist_yaw_qpos_idx,
            ]

            wrist_deviation = sum(abs(q[i]) for i in wrist_qpos_indices)
            reward -= w_wrist_ang * wrist_deviation

            wrist_qvel_indices = [
                self.left_wrist_roll_qvel_idx,
                self.left_wrist_pitch_qvel_idx,
                self.left_wrist_yaw_qvel_idx,
                self.right_wrist_roll_qvel_idx,
                self.right_wrist_pitch_qvel_idx,
                self.right_wrist_yaw_qvel_idx,
            ]

            wrist_vel = np.sum(np.abs(self.data.qvel[wrist_qvel_indices]))
            w_wrist_vel = 0.05
            reward -= w_wrist_vel * wrist_vel

            # Explicit tilt penalty when lifted
            reward -= w_tilt * tilt_angle

        # ------------------------------------------------------------------
        # 7. CONTROL COST: penalize large actions
        # ------------------------------------------------------------------
